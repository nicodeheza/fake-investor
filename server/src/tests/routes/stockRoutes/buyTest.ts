...
describe("/stock/buy route", function () {
	...
	describe("request auth", function () {
		let stockId: number, liquid: number, historyId: number;
		describe("buy a stock that is not in the database and user still don't have", function () {
			...
			it("stock must be added successfully", async function () {
				...
				stockId = (dbData as any[])[0].stock_id;
				expect((dbData as any[])[0].symbol).to.be.equal("TS");
			});
			it("ownership must be added successfully", async function () {
				const [dbData] = await db.promise().query(
					`
				...
				expect(parseFloat((dbData as any[])[0].quantity)).to.equal(reqBody.amount);
			});
			it("the correct amount of ufd must be subtracted", async function () {
			...
			it("history must be created", async function () {
				const [dbData] = await db.promise().execute(`
			...
			it("transaction must be added", async function () {
				const [dbData] = await db.promise().query(
					`
				...
				expect(transaction.buy).to.be.equal(1);
		...
		describe("buy a stock that is in the database and user already have", function () {
			...
			it("stock holding must be updated", async function () {
				const [dbData] = await db.promise().query(
					`
			    SELECT quantity FROM Ownerships WHERE stock_id=? AND user_id=1
			    `,
				...
				expect(parseFloat((dbData as any[])[0].quantity)).to.equal(reqBody.amount * 2);
			});
			it("the correct amount of ufd must be subtracted", async function () {
				const [dbData] = await db.promise().execute(
					`
			...
			it("history must be updated", async function () {
				const [dbData] = await db.promise().execute(`
				SELECT * FROM History WHERE history_date=CURRENT_DATE
				`);
				const history = (dbData as any[])[0];
			...
			it("transaction must be added", async function () {
				...
				const transactions = dbData as any[];
